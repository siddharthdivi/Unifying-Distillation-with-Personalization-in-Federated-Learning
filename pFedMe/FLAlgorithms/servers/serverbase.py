...
class Server:
    def __init__(self, device, dataset,algorithm, model, batch_size, learning_rate ,beta, lamda,
        ...
        self.args = args
        # Variable to keep track of the per-user accuracies.
        self.perUserAccs = []
    ...
    def save_results(self):
        ...
        alg = alg + "_roundNum" + "_" + str(self.args.round_num) + "_globalIters" + "_" + str(self.args.num_global_iters) + "_dataSplit" + "_" + str(self.args.dataSplitStrategy)
        if (len(self.rs_glob_acc) != 0 &  len(self.rs_train_acc) & len(self.rs_train_loss)) :
            with h5py.File("./results/"+'{}.h5'.format(alg, self.local_epochs), 'w') as hf:
                hf.create_dataset('rs_glob_acc', data=self.rs_glob_acc)
                hf.create_dataset('rs_train_acc', data=self.rs_train_acc)
                hf.create_dataset('rs_train_loss', data=self.rs_train_loss)
                hf.create_dataset('perUserAccs', data=self.perUserAccs)
                hf.close()
    ...
    def test(self):
        ...
        for c in self.users:
            ...
            userAccs.append( ((ct*1.0)/ns) )
        ids = [c.id for c in self.users]

        return ids, num_samples, tot_correct, userAccs
    ...
    # This function gets the accuracy of the personalized model on each and every user's test set. 
    def test_persionalized_model(self):
        ...
        tot_correct = []
        userAccs = []
        # Get the number of correctly classified samples, as well as the total number of samples for each user.
        for c in self.users:
    ...
    def evaluate(self):
        ...
        # TODO: Think about whether or not to get the per user accuracies for each user for the global model. 
        # self.perUserAccs.append(stats[3])
    ...
    def evaluate_personalized_model(self):
        ...
        print("Accuracy of the personalized on the test set of each user: %s." % (stats[3]))
