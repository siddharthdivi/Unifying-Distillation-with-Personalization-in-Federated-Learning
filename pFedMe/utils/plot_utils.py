...
def read_perUserAcc(alg):
    print(alg)
    hf = h5py.File("./results/"+'{}.h5'.format(alg), 'r')
    perUserAccs = np.array(hf.get('perUserAccs'))
    return perUserAccs 
...
def get_all_training_data_value(num_users=100, loc_ep1=5, Numb_Glob_Iters=10, lamb=0, learning_rate=0,beta=0,algorithms="", batch_size=0, dataset="", k= 0 , personal_learning_rate =0 ,times = 5, args = None):
    train_acc = np.zeros((times, Numb_Glob_Iters))
    train_loss = np.zeros((times, Numb_Glob_Iters))
    glob_acc = np.zeros((times, Numb_Glob_Iters))
    perUserAccs = np.zeros((times, Numb_Glob_Iters, num_users))
    ...
    for i in range(times):
        ...
        if(algorithms == "pFedMe" or algorithms == "pFedMe_p"):
            algorithms_list[i] = algorithms_list[i] + "_" + string_learning_rate + "_" + str(num_users) + "u" + "_" + str(batch_size) + "b" + "_" +str(loc_ep1) + "_"+ str(k)  + "_"+ str(personal_learning_rate) +  "_" +str(i)  + "_roundNum" + "_" + str(args.round_num) + "_globalIters" + "_" + str(args.num_global_iters)  + "_dataSplit" + "_" + str(args.dataSplitStrategy)
        else:
            algorithms_list[i] = algorithms_list[i] + "_" + string_learning_rate + "_" + str(num_users) + "u" + "_" + str(batch_size) + "b"  "_" +str(loc_ep1) +  "_" + str(i)  + "_roundNum" + "_" + str(args.round_num) + "_globalIters" + "_" + str(args.num_global_iters)  + "_dataSplit" + "_" + str(args.dataSplitStrategy) 

        train_acc[i, :], train_loss[i, :], glob_acc[i, :]  = np.array(
            simple_read_data(dataset +"_"+ algorithms_list[i]))[:, :Numb_Glob_Iters]
        
        # print ('\n Inside the func(), value of perUserAccs %s.' % (np.array(read_perUserAcc(dataset +"_"+ algorithms_list[i]))))
        if (dataset == 'Cifar10' or dataset == 'Mnist'):
            perUserAccs[i, :, :] = np.array(read_perUserAcc(dataset +"_"+ algorithms_list[i]))
        
        # print ('\n Inside the func(), after the acquisition: %s.' % (perUserAccs))
        
    return glob_acc, train_acc, train_loss, perUserAccs
...
def average_data(num_users=100, loc_ep1=5, Numb_Glob_Iters=10, lamb="", learning_rate="", beta="", algorithms="", batch_size=0, dataset = "", k = "", personal_learning_rate = "", times = 5, args = None):
    if(algorithms == "PerAvg"):
        algorithms = "PerAvg_p"
    glob_acc, train_acc, train_loss, perUserAccs = get_all_training_data_value( num_users, loc_ep1, Numb_Glob_Iters, lamb, learning_rate, beta, algorithms, batch_size, dataset, k, personal_learning_rate,times, args)
    ...
    if (not args):
        # NOTE: Added information about the global number of iterations to disinguish between different runs.
        alg = alg + "_" + str(learning_rate) + "_" + str(beta) + "_" + str(lamb) + "_" + str(num_users) + "u" + "_" + str(batch_size) + "b" + "_" + str(loc_ep1) 
    else:
        alg = alg + "_" + str(learning_rate) + "_" + str(beta) + "_" + str(lamb) + "_" + str(num_users) + "u" + "_" + str(batch_size) + "b" + "_" + str(loc_ep1)

        
    print ('Inside the average_data(), %s.' % alg)
    
    if(algorithms == "pFedMe" or algorithms == "pFedMe_p"):
        alg = alg + "_" + str(k) + "_" + str(personal_learning_rate)
    alg = alg + "_" + "avg"
    alg = alg + "_roundNum" + "_" + str(args.round_num)  + "_globalIters" + "_" + str(args.num_global_iters) + "_dataSplit" + "_" + str(args.dataSplitStrategy)
    if (len(glob_acc) != 0 &  len(train_acc) & len(train_loss)) :
        with h5py.File("./results/"+'{}.h5'.format(alg,loc_ep1), 'w') as hf:
            ...
            hf.create_dataset('perUserAccs', data=perUserAccs)
            hf.close()
